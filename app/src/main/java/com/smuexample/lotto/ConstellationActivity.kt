...
fun getShuffledLottoNumbersFormHash(str: String) : MutableList<Int>{
    val list = mutableListOf<Int>()
    for(number in 1..45){
        list.add(number)
    }

    val targetString = SimpleDateFormat("yyyy-MM-dd-HH-mm-ss-SS", Locale.KOREA)
        .format(Date()) + str
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnGoConstell.setOnClickListener {
            ...
            intent.putIntegerArrayListExtra("result",
                ArrayList(getShuffledLottoNumbersFormHash(txtConstell.text.toString())))
